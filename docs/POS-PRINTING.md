...
## 8. التشغيل في الخلفية (System Tray)

- إغلاق نافذة Agent (✕) **لا يوقف البرنامج** — يُخفى إلى أيقونة شريط المهام (بجانب الساعة).
- **انقر مرتين** على الأيقونة لإعادة فتح النافذة.
- من قائمة الأيقونة: **إنهاء البرنامج** لإيقاف الطباعة والخادم فعلياً.
- عند التشغيل بـ `npm start` من Terminal: إغلاق نافذة الـ CMD قد يوقف Agent — للإنتاج استخدم المثبّت `.exe`.
